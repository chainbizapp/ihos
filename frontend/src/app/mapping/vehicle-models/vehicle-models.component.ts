...
import { MappingApiService, VehicleModelMappingDto } from '../../core/mapping-api.service';
...
@Component({
  ...
  template: `
    <div class="p-6">
      <h1 class="text-2xl font-semibold mb-6">Vehicle Model Mappings</h1>
      ...
      @if (loading()) {
        ...
        <div class="text-gray-500 py-8 text-center">No vehicle model mappings found.</div>
      } @else {
          ...
          <table class="w-full text-sm border-collapse">
            <thead>
              <tr class="bg-gray-50 text-left text-gray-600">
                ...
                <th class="px-4 py-2 border-b font-medium">Canonical Model</th>
                <th class="px-4 py-2 border-b font-medium">Auto-Suggested</th>
            ...
            <tbody>
              @for (m of mappings(); track m.id) {
                <tr class="hover:bg-gray-50 border-b">
                  ...
                  <td class="px-4 py-2">{{ m.canonicalMakeName }} {{ m.canonicalModelName }}</td>
                  <td class="px-4 py-2">
                    @if (m.isAutoSuggested) {
                      <span class="px-2 py-0.5 rounded text-xs bg-yellow-100 text-yellow-700">Auto</span>
                    } @else {
                      <span class="px-2 py-0.5 rounded text-xs bg-green-100 text-green-700">Manual</span>
                    }
                  </td>
                  <td class="px-4 py-2">
    ...
    @if (dialogOpen()) {
      <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
        <div class="bg-white rounded-lg shadow-xl p-6 w-full max-w-md">
          <h3 class="text-lg font-semibold mb-4">{{ editingId() ? 'Edit' : 'Create' }} Vehicle Model Mapping</h3>
          ...
          <div class="mb-3">
            <label class="block text-sm font-medium text-gray-700 mb-1">Canonical Model ID</label>
            <input [(ngModel)]="form.canonicalModelId" type="text" placeholder="Vehicle model UUID"
              class="w-full border rounded px-3 py-2 text-sm font-mono" />
            <p class="text-xs text-gray-400 mt-1">Enter the UUID of the canonical vehicle model.</p>
          </div>
...
export class VehicleModelsComponent implements OnInit {
  ...
  form = { companyId: '', rawName: '', canonicalModelId: '' };
  ...
  openCreateDialog(): void {
    ...
    this.form = { companyId: '', rawName: '', canonicalModelId: '' };
    this.dialogError.set(null);
    this.dialogOpen.set(true);
  }

  openEditDialog(m: VehicleModelMappingDto): void {
    this.editingId.set(m.id);
    this.form = { companyId: m.companyId, rawName: m.rawName, canonicalModelId: m.canonicalModelId };
    this.dialogError.set(null);
    this.dialogOpen.set(true);
  }
    ...
    try {
      ...
      if (id) {
        ...
        await this.mappingApi.createVehicleModelMapping(
          this.form.companyId, this.form.rawName, this.form.canonicalModelId);
      }
      this.dialogOpen.set(false);
      await this.load();
    } catch (err: any) {
